...
HEADERS = {"User-Agent":"AI4LifeSciences/1.2 (metadata fetcher)"}; REQ_TIMEOUT = 30
...
def guess_domains(text: str, ovr: Dict[str,Any])->List[str]:
...
# --- Metadata fetchers (same as before; Crossref, PubMed, arXiv) ---
def fetch_crossref_from_doi(doi: str):
    ...
    for k in ["published-print","published-online","created","issued"]:
        if k in j and "date-parts" in j[k]: year=j[k]["date-parts"][0][0]; break
...
def main():
    ...
    with open(SRC,newline="",encoding="utf-8") as f:
        ...
        for i,row in enumerate(reader, start=1):
            ...
            if not entry["domain"]: entry["domain"]=guess_domains(blob, overrides)   # <-- title + abstract
            out.append(entry)
